...
def extract_transformer_weights(
    ...
    if not only_decoder:
        ...
        if _vocab_size % 8 != 0:
            ...
            print(
                f"token embedding is not multiple of 8, pad amount {_pad_amt} for TensorCore performance."
            )
            assert _pad_amt > 0
            src_tb = np.pad(
                src_tb, ((0, _pad_amt), (0, 0)), mode="constant", constant_values=0
            )
    ...
    if _vocab_size % 8 != 0:
        ...
        assert _pad_amt > 0
        trg_tb = np.pad(
            trg_tb, ((0, _pad_amt), (0, 0)), mode="constant", constant_values=0
        )
...
if __name__ == "__main__":
    ...
    output_lightseq_model_name = "lightseq_bart_base_embpad"  # you can rename it to "lightseq_bart_large" for large model
